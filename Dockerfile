...
RUN apt-get update     && apt-get install -y --no-install-recommends         r-base-dev=4.6.1-1.2404.1         cmake         libcurl4-openssl-dev         libxml2-dev         libssl-dev         libjpeg-dev         libpng-dev         libtiff-dev         libharfbuzz-dev         libfribidi-dev         libfontconfig1-dev         libudunits2-dev         libpq-dev         libnode-dev         liblapack-dev         libblas-dev         gfortran     && rm -rf /var/lib/apt/lists/*
...
RUN TMPDIR=/tmp/R/tmp R_LIBS_SITE=/opt/R/site-library Rscript /tmp/RPackagesInstallDocker.R     && rm /tmp/RPackagesInstallDocker.R

FROM base-deps AS base

RUN apt-get update     && apt-get install -y --no-install-recommends         r-base=4.6.1-1.2404.1         libcurl4t64         libssl3t64         libxml2         libjpeg-turbo8         libpng16-16t64         libtiff6         libharfbuzz0b         libfribidi0         libfontconfig1         libudunits2-0         libpq5         libnode109         liblapack3         libblas3     && rm -rf /var/lib/apt/lists/*
...
COPY --from=r-build /opt/R/site-library/ /opt/R/site-library/
