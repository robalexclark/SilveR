...
#Checking the design is a complete block design fo rthe Friedmans and Wilcoxon tests
if (block != "NULL") {
    RunFried <- "Y"
...
if (block == "NULL" && (statstest == "MannWhitney" || (statstest == "AllComparisons" && leng == 2) || (statstest == "CompareToControl" && leng == 2))) {
    ...
    if (dim > 2) {
        ...
        if (length(unique(eval(parse(text = paste("statdata$", response))))) < length((eval(parse(text = paste("statdata$", treatment)))))) {
            HTML("As there are ties in the responses, the asymptotic test result has been calculated.", HR = 0, align = "left")
        }
    }
}
...
if (block != "NULL" && statstest == "MannWhitney") {
    ...
    if (RunFried == "Na") {
        HTML("As not all combinations of the treatment factor and the other design (blocking) factor are present in the design, Friedman's test cannot be performed. An alternative approach is to analyse the data using the Single or Repeated Measures Parametric Analysis modules, applying the rank transformation to the responses. ", align = "left")
    }

    if (RunFried == "Nb") {
        HTML("As one of the treatment factor levels occurs multiple times with one of the other design (blocking) factor levels, Friedman's test cannot be performed. An alternative approach is to analyse the data using the Single or Repeated Measures Parametric Analysis modules, applying the rank transformation to the responses. ", align = "left")
    }
        ...
        for (s in 1:(length(unique(eval(parse(text = paste("statdata$", treatment))))) - 1)) {
            for (t in s + 1:(length(unique(eval(parse(text = paste("statdata$", treatment))))) - s)) {
                ...
                for (k in 1:(length(unique(eval(parse(text = paste("statdata$", block))))))) {
                    tempdata1x <- tempdata1[which(eval(parse(text = paste("tempdata1$", block))) == eval(parse(text = paste("tempdata1$", block)))[[k]]),]
                    tempdata2x <- tempdata2[which(eval(parse(text = paste("tempdata2$", block))) == eval(parse(text = paste("tempdata2$", block)))[[k]]),]
                    interdata[k, 1] <- tempdata1x[[1, eval(parse(text = paste("tempdata1x$", response)))]]
                    interdata[k, 2] <- tempdata2x[[1, eval(parse(text = paste("tempdata2x$", response)))]]
                }

                #Wilcoxon test
                wilcox <- wilcox.test(interdata$X1, interdata$X2, exact = FALSE, paired = TRUE)
...
if (block != "NULL" && (statstest == "MannWhitney" || (statstest == "AllComparisons" && leng == 2))) {
    dim <- length(unique(eval(parse(text = paste("statdata$", treatment)))))
