...
#Printing ANCOVA Table - note this code is reused from below
	...
	if (temx[dim(temx)[1],1] != 0)
	{
		...
		ivsanovax<-cbind(col1x,blanqx,temp2x[1],blanqx,col2x,blanqx,col3x,blanqx,col4x)
...
if(showANOVA=="Y")
{
	...
	col3<-format(round(temp2[3], 3), nsmall=3, scientific=FALSE)
	col4<-format(round(temp2[4], 2), nsmall=2, scientific=FALSE)
	col5<-format(round(temp2[5], 4), nsmall=4, scientific=FALSE)
...
#STB May 2012 capitals changed
	...
	for (i in 1:(dim(ivsanova)[1]-1)) 
	{
		if (temp2[i,5]<0.0001) 
		{
#STB Mar 2011 - formatting p-values p<0.0001
#			ivsanova[i,9]<-0.0001
...
if (allPairwiseTest == "Tukey") {
    HTML("Braun HI, ed (1994). The collected works of John W. Tukey. Vol. VIII: Multiple comparisons:1948-1983. New York: Chapman and Hall.", align = "left")
}

if (backToControlTest == "Dunnett") {
    HTML("Dunnett CW. (1955). A multiple comparison procedure for comparing several treatments with a control. Journal of the American Statistical Association, 50, 1096-1121.", align = "left")
}
...
if (showdataset == "Y") {
    ...
    statdata3 <- subset(statdata, select = -c(catfact))
