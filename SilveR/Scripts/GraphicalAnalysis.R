...
if (FirstCatFactor != "NULL" && SecondCatFactor != "NULL") {
    graphdata$catfact <- paste(eval(parse(text = paste("graphdata$", FirstCatFactor))), " ", eval(parse(text = paste("graphdata$", SecondCatFactor))), sep = "")
}
...
#Output HTML header
HTML.title("<bf>Graphics", HR = 1, align = "left")
...
if (ScatterPlot == "Y" && is.numeric(graphdata$xvarrr_IVS) == "TRUE" && is.numeric(graphdata$yvarrr_IVS) == "TRUE") {
    ...
    if (FirstCatFactor == "NULL" && SecondCatFactor == "NULL") {
        catfacttemp <- rep("Overall correlation", dim(graphdata)[1])
...
if (ScatterPlot == "Y" && jitterfunction == "Y" && is.numeric(graphdata$xvarrr_IVS) == "TRUE" && is.numeric(graphdata$yvarrr_IVS) == "TRUE") {
    add <- c("Warning: The jitter function is only applicable to categorical variables. In this case the variables are both numerical and hence no jitter has been applied.")
    HTML.title("</bf> ", HR = 2, align = "left")
    HTML.title(add, HR = 0, align = "left")
}
...
if (ScatterPlot == "Y" && infiniteslope == "Y" && is.numeric(graphdata$xvarrr_IVS) == "TRUE" && LinearFit == "Y") {
    title <- "Warning: Some of the best-fit lines have not been fitted as all the "
    title <- paste(title, XAxisVar, sep = "")
    title <- paste(title, " values are the same for all subjects in one or more of the level(s) of the categorisation factor(s).", sep = "")
    HTML.title("</bf> ", HR = 2, align = "left")
    HTML.title(title, HR = 0, align = "left")
}
...
if (ScatterPlot == "Y" && is.numeric(graphdata$xvarrr_IVS) == "TRUE" && is.numeric(graphdata$yvarrr_IVS) == "TRUE" && LinearFit == "Y") {
    #	HTMLbr()
    title <- c("Estimates of the coefficients of the linear line")
    HTML.title(title, HR = 2, align = "left")
...
if (LinearFit == "Y" && ScatterPlot == "Y" && (is.numeric(graphdata$xvarrr_IVS) == "FALSE" || is.numeric(graphdata$yvarrr_IVS) == "FALSE")) {
    title <- "Warning: As one of the variables is non-numeric, the best fit line is not included on the scatterplot."
    HTML.title("</bf> ", HR = 2, align = "left")
    HTML.title(title, HR = 0, align = "left")
}
...
if (ScatterPlot == "Y" && (is.numeric(graphdata$xvarrr_IVS) == "FALSE" || is.numeric(graphdata$yvarrr_IVS) == "FALSE")) {
    title <- "Warning: As one of the variables is non-numeric, no correlation analysis has been performed."
    HTML.title("</bf> ", HR = 2, align = "left")
    HTML.title(title, HR = 0, align = "left")
}
...
if (SEMPlot == "Y" && displaypointSEM == "Y") {
    HTML.title("<bf> ", HR = 2, align = "left")
    HTML.title("The plot includes individual observations, denoted by black dots.", HR = 0, align = "left")
}
#==============================================================================================================================================
...
if (BoxPlot == "Y" && is.numeric(graphdata$yvarrr_IVS) == TRUE) {
    ...
    if (Outliers == "Y" && displaypointBOX == "Y") {
        HTML.title("The plot includes individual observations, those that lie outside the outlier range are surrounded by an outer circle.", HR = 0, align = "left")
    }
...
if (HistogramPlot == "Y" && is.numeric(graphdata$yvarrr_IVS) == FALSE) {
    HTML.title("<bf> ", HR = 2, align = "left")
    add <- c("As the response variable is non-numeric, no graph has been produced. Please review your selection.")
    HTML.title(add, HR = 0, align = "left")
}
...
if (HistogramPlot == "Y" && ((FirstCatFactor != "NULL" && SecondCatFactor == "NULL") || (FirstCatFactor == "NULL" && SecondCatFactor != "NULL")) && GraphStyle == "Separate" && is.numeric(graphdata$yvarrr_IVS) == TRUE) {
    ...
    normaldens <- ddply(graphdata, "l_l", function(df) {
        data.frame(
        yvarrr_IVS = grid,
...
if (showdataset == "Y") {
    ...
    HTML(graphdata2, classfirstline = "second", align = "left")
}
