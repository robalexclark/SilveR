...
#marker labels size
MarkerLabelFontSize <- as.numeric(grparanum$V2[23])
if (is.na(MarkerLabelFontSize)) {
	MarkerLabelFontSize <- labsize
}
...
namereplace <- function(axistitle) {
	...
	#for (i in 1:20) {
		...
		axistitle <- gsub("ivs_pound_ivs", intToUtf8(0x00A3), axistitle)
...
NONCAT_SCAT <- function(typez) {
	...
	if (scatterlabels == "Y") {
		g8 <- g7 + geom_text_repel(aes(label = rownames(graphdata)), size = MarkerLabelFontSize,force = 10, point.padding=1)
	} else {
		g8 <- g7
	}
	if (typez == "PAIRED") {
...
ONECATSEP_SCAT <- function() {
	...
	if (scatterlabels == "Y") {
		g2 <- g1 + geom_text_repel(aes(label = rownames(graphdata)), size = MarkerLabelFontSize, force = 10, point.padding=1)
	} else {
		g2 <- g1
	}
...
OVERLAID_SCAT <- function() {
	...
	if (scatterlabels == "Y") {
		g3 <- g2 + geom_text_repel(aes(label = rownames(graphdata), color = graphdata$l_l), size = MarkerLabelFontSize,force = 10, point.padding=1, show.legend = FALSE)
	} else {
		g3 <- g2
	}
	if (ReferenceLine != "NULL") {
...
NONCAT_QQPLOT <- function(typez) {
	...
	if (scatterlabels == "Y") {
		g2 <- g + geom_text_repel(label=graphdata$name[order(graphdata$yvarrr_IVS)], stat="qq", size=MarkerLabelFontSize)
	} else {
		g2 <- g
	}
...
reference <- function (name) {
  ...
  if (name == "hassediagrams") 	  {reference <- "Michaelides D, Bate S, Chatfield M (2026). hassediagrams: Hasse Diagram of the Layout Structure and Restricted Layout Structure. doi:10.32614/CRAN.package.hassediagrams, R  package version 2.1, https://CRAN.R-project.org/package=hassediagrams."}
