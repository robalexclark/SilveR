...
add<-paste(add, c("'s "), sep="")
if (method == "Pearson")
{
add<-paste(add, c("product moment "), sep="")
}

if (method == "Kendall")
{
add<-paste(add, c("tau "), sep="")
}

if (method == "Spearman")
{
add<-paste(add, c("rho "), sep="")
}

add<-paste(add, c("correlation coefficient."), sep="")	

if (transformation != "None")
{
	add<-paste(c(" The responses have been "), transformation, sep="")
...
{
	responses<- paste(responses, c(" + "), sep="")
...
{ 
	...
	HTMLbr()
	title<-c("Non-categorised results")
...
#STB July2013
		...
		if (qt == 2)
		{
			title<- "Warning: One or more of the lines have not been fitted as there is only one pair of responses available"
			title<-paste(title, ".", sep="")
			HTML.title("</bf> ", HR=2, align="left")
			HTML.title(title, HR=0, align="left")
		}
...
#Scatterplot
	HTMLbr()
	title<-c("Matrixplot of the raw data")
	HTML.title(title, HR=2, align="left")
...
if (pValueSelected=="Y") {colsno = colsno+1}
...
rownams[1,1]="First variable"
rownams[1,2]=" "
rownams[1,3]="Second variable"
rownams[1,4]="n"
l<-5
if (estimate=="Y") 
{
...
# calculating n
		...
		if (pairn>=3)
		{
			...
			correlationTable[k,1]=resplist[i]
			correlationTable[k,2]=" vs. "
			correlationTable[k,3]=resplist[j]
			correlationTable[k,4]=pairn
...
if (k>1)
{
	...
	if (pValueSelected=="Y") 
	{
		...
		title<-paste(title, method, sep="")	
		title<-paste(title, c("'s "), sep="")
	...
	HTML.title(title, HR=2, align="left")
	HTML(correlationTable2, classfirstline="second", align="left")
...
#reorder levels for categorised plot
			...
			{
				HTML.title("<bf> ", HR=2, align="left")
				HTML.title("<bf> Warning: The colours employed on this plot are not consistent with others within this module as it was not possible to fit a regression line for one or more groups.", HR=0, align="left")
			}
...
if (qt == 2)
{
	title<- "Warning: One or more of the lines have not been fitted as there is only one pair of responses available at one or more of"
	title<-paste(title, " the level(s) of the categorisation factor.", sep="")
	HTML.title("</bf> ", HR=2, align="left")
	HTML.title(title, HR=0, align="left")
}
...
#Table column names
rownams<-matrix(ncol=colsno,nrow=1)
rownams[1,1]="Categorisation factor"
...
#STB Aug 2011 - removing lines with infinite slope
				...
				correlationTable[k,1]=unique(levels(as.factor(statdata$catfact)))[p]	
...
if (k>2)
{
	...
	for(i in 1:(dim(correlationTable2)[1]))
	{
		...
		if (correlationTable2[i,corp] <= (1-CIval) || correlationTable2[i,corp] == "< 0.0001" )
		{
			if (inte==1)
			{
				...
				add<-paste(add, CIval3, sep="")
				add<-paste(add, "% level: ", sep="")
				add<-paste(add, correlationTable2[i,2], " vs. ", correlationTable2[i,4], " (categorisation factor(s) level ", sep="")
				add<-paste(add, correlationTable2[i,1], ")",sep="")
			} else {
				inte<-inte+1
				add<-paste(add, ", ", sep="")
				add<-paste(add, correlationTable2[i,2], " vs. ", correlationTable2[i,4], " (categorisation factor(s) level ", sep="")
	...
	{
		HTML.title("<bf> ", HR=2, align="left")
		HTML.title("<bf>   Note that there is only one or two pairs of responses available for one or more of the level(s) of the categorisation factor(s) and hence no correlation analysis can be performed for the level(s) of the categorisation factor(s). ", HR=0, align="left")
	}
	...
	{
		HTML.title("<bf> ", HR=2, align="left")
		HTML.title("<bf> The test statistic is based on Pearson's product moment correlation coefficient cor(x, y) and follows a t distribution with length(x)-2 degrees of freedom if the samples follow independent normal distributions, see Snedecor and Cochran (1989). ", HR=0, align="left")
	}

	if (method == "Kendall") 
	{
		HTML.title("<bf> ", HR=2, align="left")
		HTML.title("<bf> This test is a rank-based measure of association and can be used when the data are not necessarily from a bivariate normal distribution, see Hollander and Wolfe (1973). 
		The test statistic is the estimate scaled to zero mean and unit variance, and is approximately normally distributed. ", HR=0, align="left")
	}
	...
	if (method == "Spearman") 
	{
		HTML.title("<bf> ", HR=2, align="left")
		HTML.title("<bf> This test is a rank-based measure of association and can be used when the data are not necessarily from a bivariate normal distribution, see Hollander and Wolfe (1973).
		The p-values are computed using algorithm AS 89, see Best and Roberts (1975). ", HR=0, align="left")
	}

} else {
	HTMLbr()
	HTML.title("<bf>Warning", HR=2, align="left")
	add<-paste(c("As the replication of every combination of the levels of the categorical factor(s) for all pairs of responses is two or less, no correlation analysis has been performed."))
	HTML.title("</bf> ", HR=2, align="left")
	HTML.title(add, HR=0, align="left")
	}
