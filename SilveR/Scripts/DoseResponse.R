...
#Output HTML header
HTML.title("<bf>Dose-Response Analysis", HR = 1, align = "left")
...
if (DoseResponseType == "FromEquation") {
    ...
    if (grepl("x", Equationx, fixed = TRUE, ignore.case = FALSE) == FALSE) {
        title <- c("Warning")
        HTML.title(title, HR = 2, align = "left")
        add <- c("The equation entered is not of the form y=f(x). The equation must contain a (lower case) x variable for the analysis to proceed.")
        HTML.title("</bf> ", HR = 2, align = "left")
        HTML.title(add, HR = 0, align = "left")
    }
...
if (DoseResponseType == "FourParameter") {
    ...
    if (unique(eval(parse(text = paste("statdata$", DoseVar))))[1] == 0) {
        ...
        if (MinCoeff == "NULL" && MaxCoeff == "NULL" && SlopeCoeff == "NULL" && ECIDCoeff == "NULL" && DoseTransform != "Loge") {
            dosefit <- nls(responsezzzz ~ D + (A - D) / (1 + 10 ^ ((C - logconczzzz) * B)), start = list(A = maxp, B = slopep, C = ed50p, D = minp), data = statdata)
        } else if (MinCoeff == "NULL" && MaxCoeff == "NULL" && SlopeCoeff == "NULL" && ECIDCoeff == "NULL" && DoseTransform == "Loge") {
            dosefit <- nls(responsezzzz ~ D + (A - D) / (1 + exp((C - logconczzzz) * B)), start = list(A = maxp, B = slopep, C = ed50p, D = minp), data = statdata)
        } else if (MinCoeff == "NULL" && MaxCoeff != "NULL" && SlopeCoeff == "NULL" && ECIDCoeff == "NULL" && DoseTransform != "Loge") {
            dosefit <- nls(responsezzzz ~ D + (MaxCoeffp - D) / (1 + 10 ^ ((C - logconczzzz) * B)), start = list(B = slopep, C = ed50p, D = minp), data = statdata)
        } else if (MinCoeff == "NULL" && MaxCoeff != "NULL" && SlopeCoeff == "NULL" && ECIDCoeff == "NULL" && DoseTransform == "Loge") {
            dosefit <- nls(responsezzzz ~ D + (MaxCoeffp - D) / (1 + exp((C - logconczzzz) * B)), start = list(B = slopep, C = ed50p, D = minp), data = statdata)
        } else if (MinCoeff == "NULL" && MaxCoeff == "NULL" && SlopeCoeff != "NULL" && ECIDCoeff == "NULL" && DoseTransform != "Loge") {
            dosefit <- nls(responsezzzz ~ D + (A - D) / (1 + 10 ^ ((C - logconczzzz) * SlopeCoeffp)), start = list(A = maxp, C = ed50p, D = minp), data = statdata)
        } else if (MinCoeff == "NULL" && MaxCoeff == "NULL" && SlopeCoeff != "NULL" && ECIDCoeff == "NULL" && DoseTransform == "Loge") {
            dosefit <- nls(responsezzzz ~ D + (A - D) / (1 + exp((C - logconczzzz) * SlopeCoeffp)), start = list(A = maxp, C = ed50p, D = minp), data = statdata)
        } else if (MinCoeff == "NULL" && MaxCoeff == "NULL" && SlopeCoeff == "NULL" && ECIDCoeff != "NULL" && DoseTransform != "Loge") {
            dosefit <- nls(responsezzzz ~ D + (A - D) / (1 + 10 ^ ((ECIDCoeffp - logconczzzz) * B)), start = list(A = maxp, B = slopep, D = minp), data = statdata)
        } else if (MinCoeff == "NULL" && MaxCoeff == "NULL" && SlopeCoeff == "NULL" && ECIDCoeff != "NULL" && DoseTransform == "Loge") {
            dosefit <- nls(responsezzzz ~ D + (A - D) / (1 + exp((ECIDCoeffp - logconczzzz) * B)), start = list(A = maxp, B = slopep, D = minp), data = statdata)
        } else if (MinCoeff != "NULL" && MaxCoeff == "NULL" && SlopeCoeff == "NULL" && ECIDCoeff == "NULL" && DoseTransform != "Loge") {
            dosefit <- nls(responsezzzz ~ MinCoeffp + (A - MinCoeffp) / (1 + 10 ^ ((C - logconczzzz) * B)), start = list(A = maxp, B = slopep, C = ed50p), data = statdata)
        } else if (MinCoeff != "NULL" && MaxCoeff == "NULL" && SlopeCoeff == "NULL" && ECIDCoeff == "NULL" && DoseTransform == "Loge") {
            dosefit <- nls(responsezzzz ~ MinCoeffp + (A - MinCoeffp) / (1 + exp((C - logconczzzz) * B)), start = list(A = maxp, B = slopep, C = ed50p), data = statdata)
        } else if (MinCoeff == "NULL" && MaxCoeff != "NULL" && SlopeCoeff != "NULL" && ECIDCoeff == "NULL" && DoseTransform != "Loge") {
            dosefit <- nls(responsezzzz ~ D + (MaxCoeffp - D) / (1 + 10 ^ ((C - logconczzzz) * SlopeCoeffp)), start = list(C = ed50p, D = minp), data = statdata)
        } else if (MinCoeff == "NULL" && MaxCoeff != "NULL" && SlopeCoeff != "NULL" && ECIDCoeff == "NULL" && DoseTransform == "Loge") {
            dosefit <- nls(responsezzzz ~ D + (MaxCoeffp - D) / (1 + exp((C - logconczzzz) * SlopeCoeffp)), start = list(C = ed50p, D = minp), data = statdata)
        } else if (MinCoeff == "NULL" && MaxCoeff != "NULL" && SlopeCoeff == "NULL" && ECIDCoeff != "NULL" && DoseTransform != "Loge") {
            dosefit <- nls(responsezzzz ~ D + (MaxCoeffp - D) / (1 + 10 ^ ((ECIDCoeffp - logconczzzz) * B)), start = list(B = slopep, D = minp), data = statdata)
        } else if (MinCoeff == "NULL" && MaxCoeff != "NULL" && SlopeCoeff == "NULL" && ECIDCoeff != "NULL" && DoseTransform == "Loge") {
            dosefit <- nls(responsezzzz ~ D + (MaxCoeffp - D) / (1 + exp((ECIDCoeffp - logconczzzz) * B)), start = list(B = slopep, D = minp), data = statdata)
        } else if (MinCoeff != "NULL" && MaxCoeff != "NULL" && SlopeCoeff == "NULL" && ECIDCoeff == "NULL" && DoseTransform != "Loge") {
            dosefit <- nls(responsezzzz ~ MinCoeffp + (MaxCoeffp - MinCoeffp) / (1 + 10 ^ ((C - logconczzzz) * B)), start = list(B = slopep, C = ed50p), data = statdata)
        } else if (MinCoeff != "NULL" && MaxCoeff != "NULL" && SlopeCoeff == "NULL" && ECIDCoeff == "NULL" && DoseTransform == "Loge") {
            dosefit <- nls(responsezzzz ~ MinCoeffp + (MaxCoeffp - MinCoeffp) / (1 + exp((C - logconczzzz) * B)), start = list(B = slopep, C = ed50p), data = statdata)
        } else if (MinCoeff == "NULL" && MaxCoeff == "NULL" && SlopeCoeff != "NULL" && ECIDCoeff != "NULL" && DoseTransform != "Loge") {
            dosefit <- nls(responsezzzz ~ D + (A - D) / (1 + 10 ^ ((ECIDCoeffp - logconczzzz) * SlopeCoeffp)), start = list(A = maxp, D = minp), data = statdata)
        } else if (MinCoeff == "NULL" && MaxCoeff == "NULL" && SlopeCoeff != "NULL" && ECIDCoeff != "NULL" && DoseTransform == "Loge") {
            dosefit <- nls(responsezzzz ~ D + (A - D) / (1 + exp((ECIDCoeffp - logconczzzz) * SlopeCoeffp)), start = list(A = maxp, D = minp), data = statdata)
        } else if (MinCoeff != "NULL" && MaxCoeff == "NULL" && SlopeCoeff != "NULL" && ECIDCoeff == "NULL" && DoseTransform != "Loge") {
            dosefit <- nls(responsezzzz ~ MinCoeffp + (A - MinCoeffp) / (1 + 10 ^ ((C - logconczzzz) * SlopeCoeffp)), start = list(A = maxp, C = ed50p), data = statdata)
        } else if (MinCoeff != "NULL" && MaxCoeff == "NULL" && SlopeCoeff != "NULL" && ECIDCoeff == "NULL" && DoseTransform == "Loge") {
            dosefit <- nls(responsezzzz ~ MinCoeffp + (A - MinCoeffp) / (1 + exp((C - logconczzzz) * SlopeCoeffp)), start = list(A = maxp, C = ed50p), data = statdata)
        } else if (MinCoeff != "NULL" && MaxCoeff == "NULL" && SlopeCoeff == "NULL" && ECIDCoeff != "NULL" && DoseTransform != "Loge") {
            dosefit <- nls(responsezzzz ~ MinCoeffp + (A - MinCoeffp) / (1 + 10 ^ ((ECIDCoeffp - logconczzzz) * B)), start = list(A = maxp, B = slopep), data = statdata)
        } else if (MinCoeff != "NULL" && MaxCoeff == "NULL" && SlopeCoeff == "NULL" && ECIDCoeff != "NULL" && DoseTransform == "Loge") {
            dosefit <- nls(responsezzzz ~ MinCoeffp + (A - MinCoeffp) / (1 + exp((ECIDCoeffp - logconczzzz) * B)), start = list(A = maxp, B = slopep), data = statdata)
        } else if (MinCoeff == "NULL" && MaxCoeff != "NULL" && SlopeCoeff != "NULL" && ECIDCoeff != "NULL" && DoseTransform != "Loge") {
            dosefit <- nls(responsezzzz ~ D + (MaxCoeffp - D) / (1 + 10 ^ ((ECIDCoeffp - logconczzzz) * SlopeCoeffp)), start = list(D = minp), data = statdata)
        } else if (MinCoeff == "NULL" && MaxCoeff != "NULL" && SlopeCoeff != "NULL" && ECIDCoeff != "NULL" && DoseTransform == "Loge") {
            dosefit <- nls(responsezzzz ~ D + (MaxCoeffp - D) / (1 + exp((ECIDCoeffp - logconczzzz) * SlopeCoeffp)), start = list(D = minp), data = statdata)
        } else if (MinCoeff != "NULL" && MaxCoeff != "NULL" && SlopeCoeff != "NULL" && ECIDCoeff == "NULL" && DoseTransform != "Loge") {
            dosefit <- nls(responsezzzz ~ MinCoeffp + (MaxCoeffp - MinCoeffp) / (1 + 10 ^ ((C - logconczzzz) * SlopeCoeffp)), start = list(C = ed50p), data = statdata)
        } else if (MinCoeff != "NULL" && MaxCoeff != "NULL" && SlopeCoeff != "NULL" && ECIDCoeff == "NULL" && DoseTransform == "Loge") {
            dosefit <- nls(responsezzzz ~ MinCoeffp + (MaxCoeffp - MinCoeffp) / (1 + exp((C - logconczzzz) * SlopeCoeffp)), start = list(C = ed50p), data = statdata)
        } else if (MinCoeff != "NULL" && MaxCoeff != "NULL" && SlopeCoeff == "NULL" && ECIDCoeff != "NULL" && DoseTransform != "Loge") {
            dosefit <- nls(responsezzzz ~ MinCoeffp + (MaxCoeffp - MinCoeffp) / (1 + 10 ^ ((ECIDCoeffp - logconczzzz) * B)), start = list(B = slopep), data = statdata)
        } else if (MinCoeff != "NULL" && MaxCoeff != "NULL" && SlopeCoeff == "NULL" && ECIDCoeff != "NULL" && DoseTransform == "Loge") {
            dosefit <- nls(responsezzzz ~ MinCoeffp + (MaxCoeffp - MinCoeffp) / (1 + exp((ECIDCoeffp - logconczzzz) * B)), start = list(B = slopep), data = statdata)
        } else if (MinCoeff != "NULL" && MaxCoeff == "NULL" && SlopeCoeff != "NULL" && ECIDCoeff != "NULL" && DoseTransform != "Loge") {
            dosefit <- nls(responsezzzz ~ MinCoeffp + (A - MinCoeffp) / (1 + 10 ^ ((ECIDCoeffp - logconczzzz) * SlopeCoeffp)), start = list(A = maxp), data = statdata)
        } else if (MinCoeff != "NULL" && MaxCoeff == "NULL" && SlopeCoeff != "NULL" && ECIDCoeff != "NULL" && DoseTransform == "Loge") {
            dosefit <- nls(responsezzzz ~ MinCoeffp + (A - MinCoeffp) / (1 + exp((ECIDCoeffp - logconczzzz) * SlopeCoeffp)), start = list(A = maxp), data = statdata)
        } else if (MinCoeff != "NULL" && MaxCoeff != "NULL" && SlopeCoeff != "NULL" && ECIDCoeff != "NULL" && DoseTransform != "Loge") {
                    ...
                    if (ECIDCoeff != "NULL" && DoseTransform == "Loge") {
                        ...
                        if (ECIDCoeff == "NULL" && DoseTransform == "Log10") {
                            ...
                            if (SlopeCoeff == "NULL") {
                                ...
                                #STB Dec 2011 Formatting to 3dp
                                add <- paste(add, format(round(B, 3), nsmall = 3, scientific = FALSE), sep = "")
                                add <- paste(add, ". ", sep = "")
                            }
                            ...
                            if (ECIDCoeff == "NULL") {
                                ...
                                #STB Dec 2011 Formatting to 3dp
                                add <- paste(add, format(round(C, 3), nsmall = 3, scientific = FALSE), sep = "")
                                add <- paste(add, ". ", sep = "")
                            }
                            ...
                            if (QCResponse != "NULL" && QCDose != "NULL") {
                                ...
                                for (i in 1:length(tempdata3)) { Type[i] = " Quality controls" }
