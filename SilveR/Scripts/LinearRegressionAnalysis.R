...
if (nocontfactors == 1) {
	...
	HTML(esttab, , align="left" , classfirstline="second", row.names="FALSE")
}
...
if(covariateModel != "NULL") {
	...
	HTML("These issues are discussed in more detail in Morris (1999).", align="left")
		...
		HTML("Tip: In order to decide whether it is helpful to fit the covariate, consider the relationship between the response and the covariate. It is only worth fitting the covariate if there is a strong positive (or negative) relationship between them: i.e., the lines on the plot should not be horizontal. This is discussed in more detail in Morris (1999).", align="left")
	}
}
...
if(showANOVA=="Y") {
	...
	if (tempxX[dim(tempxX)[1],1] == 0) {
		HTML("The ANOVA have not been calculated as there are zero residual degrees of freedom when all terms are included in the statistical model.", align="left")
		quit()
	} else {
		...
		for(i in 1:(dim(ivsanova)[1]-1)) {
			if (ivsanova[i,9]<= (1-sig)) {
				if (inte==1) {
					inte<-inte+1
					add<-paste(add, ": There is a statistically significant effect of ", rownames(ivsanova)[i], sep="")
				} else {
					inte<-inte+1
...
if (notreatfactors==1) {
	add<-paste(add, " and categorical treatment factor ", treatFactors,  sep="")
} 
	...
	if(noblockfactors>1) # there is two or more blocking factors {
		for (i in 1:noblockfactors) {
			if (i<noblockfactors-1) {
				...
				add<-paste(add, BlockList[i+1], " and ", sep="")
			} else if (i==noblockfactors) {
				add<-paste(add, BlockList[i+1], sep="")
			}
		}
